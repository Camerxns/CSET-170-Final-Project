    ...
    match current_user.account_type():
        ...
        case "CUSTOMER":
            result = db.session.execute(text(
                f"select title, description, product_image, category from Carts natural join Cart_Items join Products using(product_id) where customer_id = {current_user.user_id};")).all()
...
def products_page(product_id):
    ...
    price = db.session.execute(text(
        f"SELECT price FROM Vendor_Products WHERE vendor_product_id=(SELECT vendor_product_id FROM Vendor_Products WHERE vendor_id={vendor_id} AND product_id={product_id})")).first()
